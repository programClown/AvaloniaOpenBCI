...
internal class Router : CorePropertyChanged, IRouter, IDisposable
{
    ...
    public Router(
        ...
        Func<IRoutableHostScreen, RouteResolution, RouterNavigationOptions, Navigation> getNavigation
    )
    {
        _logger = logger;
    ...
    private async Task<bool> RequestClose(IRoutableScreen screen, NavigationArguments args)
    {
        ...
        if (
            screen is IRoutableHostScreen hostScreen
            && hostScreen.InternalScreen != null
            && !await RequestClose(hostScreen.InternalScreen, args)
        )
            return false;
        ...
        _logger.Debug(
            "Navigation to {Path} cancelled during RequestClose by {Screen}",
            args.Path,
            screen.GetType().Name
        );
    ...
    {
        if (_root == null)
            throw new Exception("Cannot navigate without a root having been set");
    ...
    /// <inheritdoc />
    public void SetRoot<TScreen>(RoutableHostScreen<TScreen> root)
    ...
    /// <inheritdoc />
    public void SetRoot<TScreen, TParam>(RoutableHostScreen<TScreen, TParam> root)
        where TScreen : RoutableScreen
        where TParam : new()
    {
        _root = root;
    ...
    private void MainWindowServiceOnMainWindowOpened(object? sender, EventArgs e)
    {
        if (_previousWindowRoute != null && _currentRouteSubject.Value == "blank")
            Dispatcher.UIThread.InvokeAsync(
                async () =>
                    await Navigate(
                        _previousWindowRoute,
                        new RouterNavigationOptions { AddToHistory = false, EnableLogging = false }
                    )
            );
    }
    ...
    private void MainWindowServiceOnMainWindowClosed(object? sender, EventArgs e)
    {
        ...
        Dispatcher.UIThread.InvokeAsync(
            async () =>
                await Navigate("blank", new RouterNavigationOptions { AddToHistory = false, EnableLogging = false })
        );
    }
}
