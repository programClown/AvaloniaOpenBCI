...
internal class Navigation
{
    ...
    public Navigation(
        ...
        ILogger logger,
        IRoutableHostScreen root,
        RouteResolution resolution,
        RouterNavigationOptions options
    )
    {
        _service = service;
        _logger = logger;
    ...
    private async Task NavigateResolution(
        RouteResolution resolution,
        NavigationArguments args,
        IRoutableHostScreen host
    )
    {
        if (Cancelled)
            return;
        ...
        if (
            _options.RecycleScreens
            && host.RecycleScreen
            && host.InternalScreen != null
            && host.InternalScreen.GetType() == resolution.ViewModel
        )
            screen = host.InternalScreen;
        else
            screen = await Task.Run(() => resolution.GetViewModel(_service));

        // If resolution has a child, ensure the screen can host it
        if (resolution.Child != null && screen is not IRoutableHostScreen)
            throw new Exception(
                $"Route resolved with a child but view model of type {resolution.ViewModel} is does mot implement {nameof(IRoutableHostScreen)}."
            );
